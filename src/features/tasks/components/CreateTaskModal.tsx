...
export function CreateTaskModal({
    ...
    if (created) {
      if (repeatEnabled && repeatStart) {
        try {
          ...
          console.warn('[CreateTaskModal] Auto Repeat creation failed — user can set it from task detail view')
        }
      }
      if (andAnother) {
  ...
  return (
    ...
    <RepeatModal
      ...
      savedRepeat={repeatEnabled ? {
        id: '__pending__',
        frequency: repeatFreq,
        startDate: repeatStart,
        endDate: repeatEnd || null,
        repeatOnDay: repeatOnDay ? Number(repeatOnDay) : null,
      ...
      onSave={async (input) => {
        ...
        setRepeatFreq(input.frequency)
        setRepeatStart(input.startDate)
        setRepeatEnd(input.endDate ?? '')
        setRepeatOnDay(String(input.repeatOnDay ?? ''))
        setRepeatOnWeekdays(input.repeatOnWeekdays ?? [])
      }}
      onRemove={repeatEnabled ? async () => {
        setRepeatEnabled(false)
        setRepeatStart(''); setRepeatEnd(''); setRepeatOnDay(''); setRepeatOnWeekdays([])
      } : undefined}
    />
